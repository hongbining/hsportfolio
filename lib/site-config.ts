...
export const siteConfig = {
  ...
  about: [
    "저는 8년 이상의 경력을 가진 풀스택 개발자로, Spring Boot 기반 웹 서비스와 엔터프라이즈 시스템 개발 경험을 보유하고 있습니다. SI·공공·콘텐츠 플랫폼·GIS·B2B 커머스 등 다양한 도메인을 거치며, 단순 개발을 넘어 시스템 설계와 운영까지 수행해왔습니다.",
    "최근에는 건강기능식품 B2B 폐쇄몰(KH)을 기획부터 설계·개발·배포·운영까지 혼자 만들고 있습니다. 사업자 인증을 거친 회원만 거래하고 비로그인 사용자에게는 도매가를 노출하지 않는 폐쇄몰 구조를 설계했고, 거래처별 가격 정책과 주문·배송·정산까지 도매 거래 전 과정을 Spring Boot와 Next.js로 구현했습니다.",
    "CJ 골프장 관리 시스템에서는 GeoTIFF 기반 지도 이미지 처리와 GDAL 타일링, AWS S3·CloudFront 기반 대용량 이미지 처리 구조를 설계·구현하고 Docker 배포 환경까지 구성했습니다. TV조선에서는 VOD·결제(Inicis)·구독 정기결제 배치 등 콘텐츠 플랫폼 핵심 기능을 개발했고, 큐브에이에서는 CMS·관리자 시스템을 총괄하며 팀 리딩 역할을 수행했습니다.",
    "특히 동료와의 소통과 고객 관리를 중요하게 생각합니다. 문제를 함께 공유해 해결하며 프로젝트를 이끌어 왔고, 단순 기능 개발보다 시스템 구조 개선·자동화·운영 효율에 관심이 많아 지속적으로 확장 가능한 아키텍처를 지향합니다.",
  ],
  nav,
  socials,
  techStack,
